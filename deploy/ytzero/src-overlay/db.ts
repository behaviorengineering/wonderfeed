...
if (databaseConfig.engine === "postgres") {
  ...
  // imgcache may have skipped DDL while settings was missing (import race during
  // cold-start migration). Re-run after the Postgres schema is present.
  const { ensureImageCacheSchema } = await import("./imgcache");
