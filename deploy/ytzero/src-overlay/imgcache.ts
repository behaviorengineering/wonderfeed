...
/**
 * Ensure the image_cache table exists for the active engine.
 *
 * On PostgreSQL cold start, `db.ts` may still be migrating from SQLite. Creating
 * this table early makes `migrateSQLiteToPostgres` fail its empty-target check
 * (Bun can evaluate this module while `db.ts` is suspended on top-level await).
 * Skip until `settings` exists; callers after migration invoke this again.
 */
export async function ensureImageCacheSchema(): Promise<void> {
  if (database.engine === "postgres") {
    ...
    if (!schema?.settings_table) return;
  }
